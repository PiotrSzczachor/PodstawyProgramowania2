...
VectoredList::VectoredList(VectoredList &&v)  noexcept {
    head = v.head;
    tail = v.tail;
    size = v.size;
    capacity = v.capacity;
    ...
    v.capacity = 0;
    v.size = 0;
    cout<<"Teraz pracuje konstruktor przenoszacy";
}

VectoredList &VectoredList::operator+(const VectoredList &rhs) {
    size = rhs.get_size() - 10;
...
void VectoredList::assign(vector<string> data) {
    this->clean_vector();
    this->size = data.size();
